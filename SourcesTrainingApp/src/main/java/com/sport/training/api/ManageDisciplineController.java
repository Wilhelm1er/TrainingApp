...
public class ManageDisciplineController {
	...
	public String updateDiscipline(@Valid @ModelAttribute DisciplineDTO disciplineDTO, Model model, HttpServletRequest request, @CookieValue("JSESSIONID") String cookie) {
	...
	public String deleteDiscipline(@PathVariable String disciplineId, Model model, HttpServletRequest request, @CookieValue("JSESSIONID") String cookie) {
	...
	private DisciplineDTO retrieveDiscipline(String disciplineId) {
		return this.webClient
					...
					.retrieve()
					.bodyToMono(DisciplineDTO.class)
					.block();
	}
	...
	private String restUpdateDiscipline(DisciplineDTO disciplineDTO, HttpServletRequest request, String cookie) {
		...
		return this.webClient
					.put()
					.uri("/discipline/"+disciplineDTO.getId())
					.header("X-CSRF-TOKEN", sessionToken.getToken())		// oubli => 403 FORBIDDEN
					.cookie("JSESSIONID",cookie)							// oubli => 302 FOUND
					.body(Mono.just(disciplineDTO), DisciplineDTO.class)
					.retrieve()
					// Facilitateur de deboggage
					.onStatus(HttpStatus::isError, clientResponse ->
	                Mono.error(new Exception("error : "+clientResponse.statusCode()))
							)
					.onStatus(HttpStatus::is3xxRedirection, clientResponse ->
	                Mono.error(new Exception("is3xxRedirection : "+clientResponse.statusCode()))
							)
					.onStatus(HttpStatus::is4xxClientError, clientResponse ->
	                Mono.error(new Exception("is4xxClientError : "+clientResponse.statusCode()))
							)
					.onStatus(HttpStatus::is5xxServerError, clientResponse ->
	                Mono.error(new Exception("is5xxServerError : "+clientResponse.statusCode()))
	            		)
