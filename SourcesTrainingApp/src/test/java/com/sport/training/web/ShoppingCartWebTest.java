...
public class ShoppingCartWebTest {
	...
	public void testCheckoutAsUser() {
		try {
			this.mockMvc.perform(get("/checkout")).andExpect(status().isOk()).andExpect(content()
					.string(containsString("com.sport.training.exception.CreateException : Order object is empty")));
			this.mockMvc.perform(logout());
		} catch (Exception e) {
			fail("No exception expected");
		}
