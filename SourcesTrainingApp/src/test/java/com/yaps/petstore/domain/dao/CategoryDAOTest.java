...
public class CategoryDAOTest {
    ...
    public void testDomainUpdateCategory() throws Exception {
        ...
        Discipline categoryUpdated = null;
        try {
    ...
    //=         Private Methods        =
    //==================================
    private Discipline findCategory(final String id) throws NoSuchElementException {
        final Discipline category = categoryRepository.findById(id).get();
        return category;
    }

    private int findAllCategories() throws FinderException {
        try {
            return ((Collection<Discipline>) categoryRepository.findAll()).size();
        } catch (Exception e) {
            return 0;
        }
    }
    ...
    private void createCategory(final String id) throws CreateException {
        final Discipline category = new Discipline(id, "name" + id, "description" + id);
        categoryRepository.save(category);
    }

    private void updateCategory(final Discipline category, final String id) throws UpdateException, ObjectNotFoundException {
    ...
    private void removeCategory(final String id) throws ObjectNotFoundException, RemoveException {
        final Discipline category = new Discipline(id);
        categoryRepository.delete(category);
    }

    private void checkCategory(final Discipline category, final String id) {
