...
import com.sport.training.domain.dao.OrderRepository;
import com.sport.training.domain.dto.DisciplineDTO;
import com.sport.training.domain.dto.EventDTO;
import com.sport.training.domain.dto.OrderDTO;
import com.sport.training.domain.dto.OrderLineDTO;
import com.sport.training.domain.service.SportService;
import com.sport.training.domain.service.CounterService;
import com.sport.training.domain.service.OrderService;
...
//		final Long id = Long.parseLong(counterService.getUniqueId(COUNTER_NAME));
	...
	public void testServiceCreateOrderLineWithInvalidOrderValues() throws Exception {
		...
		// Creates an object
		createItem(id);
		EventDTO itemDTO = catalogService.findEvent(id);
	...
	private OrderDTO createOrderDTO(final long id) throws CreateException, RemoteException, FinderException {
		...
		final DisciplineDTO categoryDTO = new DisciplineDTO("cat" + categoryId, "name" + categoryId,
				"description" + categoryId);
		catalogService.createDiscipline(categoryDTO);
		// Create Product
		final String productId = counterService.getUniqueId("Product");
		final ActivityDTO productDTO = new ActivityDTO("prod" + productId, "name" + productId, "description" + productId);
...
//        itemDTO.setImagePath("imagePath" + id);
	...
	private void deleteOrder(final long orderId) throws RemoveException, RemoteException, FinderException {
		...
		catalogService.deleteEvent("item" + itemBeDeleted);
		catalogService.deleteActivity("prod" + prodIBeDeleted);
		catalogService.deleteDiscipline("cat" + catIBeDeleted);
	}
	...
	protected void createItem(final String id) throws CreateException, RemoveException, FinderException {
		...
		try {
			...
			catalogService.deleteActivity(productDTO.getId());
			catalogService.deleteDiscipline(categoryDTO.getId());
			throw e;
		}
	}

	private void deleteItem(final String id) throws RemoveException, FinderException {
		final EventDTO itemDTO = catalogService.findEvent(id);
		final String productId = itemDTO.getProductDTO().getId();
		final ActivityDTO productDTO = catalogService.findActivity(productId);
		final String categoryId = productDTO.getCategoryDTO().getId();
		catalogService.deleteEvent(id);
		catalogService.deleteActivity(productId);
		catalogService.deleteDiscipline(categoryId);
	}
